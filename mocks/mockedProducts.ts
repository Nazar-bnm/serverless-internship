...
const image = (): string => fakerHelper.randomImage();

const tag = (faker: Faker.FakerStatic): string => faker.commerce.productAdjective();

const share = (faker: Faker.FakerStatic): Share => ({
  name: faker.internet.domainWord(), // 'facebook'
  url: faker.internet.url()
});

const product = (faker: Faker.FakerStatic): Product => {
  const category = faker.random.arrayElement(Object.values(categories));
  const subCategory = faker.random.arrayElement(category.subCategories);
...
export default fakerHelper.timesObj(PRODUCTS_NUM, product);
